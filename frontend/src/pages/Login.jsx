...
export default function Login() {
  ...
  const submitOrg = async (e) => {
    ...
    try {
      ...
      toast.error(parseApiError(err, { 401: 'Incorrect email or password.', 429: 'Too many attempts. Please wait 15 minutes.', default: 'Sign-in failed. Please try again.' }));
  ...
  const submitMember = async (e) => {
    ...
    try {
      ...
      toast.error(parseApiError(err, { 401: 'Incorrect email or password.', 403: 'Your organisation account is suspended.', default: 'Sign-in failed. Please try again.' }));
    } finally { setLoading(false); }
  };
  ...
  return (
    <div style={s.page}>
      <style>{`
        ...
        .tab-btn:hover { color: ${dark ? '#e2e8f0' : '#0f172a'} !important; }
        .sign-btn:hover:not(:disabled) { background: #0284c7 !important; }
        .theme-toggle-login:hover { opacity: 0.7; }
        ...
        <div style={s.formSide}>
          <div style={s.card}>
            ...
            <div style={s.cardLogo}>
              <div style={s.logoMark}>L</div>
              <span style={{ fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>Lucred Credit Engine</span>
            </div>
            ...
            <div style={s.tabs}>
              {[['org', 'Organisation'], ['member', 'Team Member']].map(([t, label]) => (
                <button key={t} type="button" className="tab-btn" onClick={() => setTab(t)} style={{ ...s.tab, ...(tab === t ? s.tabActive : {}) }}>{label}</button>
              ))}
            </div>
            ...
            <form onSubmit={tab === 'org' ? submitOrg : submitMember} style={{ marginTop: 24 }}>
              <div style={s.field}>
                ...
                <input className="auth-input" style={s.input} type="email" placeholder="you@mfi.ng" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
              </div>
              <div style={s.field}>
                <label style={s.label}>Password</label>
                <input className="auth-input" style={s.input} type="password" placeholder="••••••••" required value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} />
              </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: dark ? '#475569' : '#64748b' }}>
              <Link to="/forgot-password" style={{ color: dark ? '#64748b' : '#475569', textDecoration: 'none' }}>Forgot password?</Link>
              {tab === 'org' && (
...
function makeStyles(dark) {
  ...
  return {
    ...
    formSide:  { display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem 2rem', position: 'relative' },
    card:      { width: '100%', maxWidth: 400 },
    cardLogo:  { display: 'none', alignItems: 'center', gap: 10, marginBottom: 36 },
    heading:   { fontSize: 26, fontWeight: 800, color: text, margin: '0 0 6px', letterSpacing: -0.5 },
    sub:       { fontSize: 14, color: sub, margin: '0 0 28px' },
    tabs:      { display: 'flex', background: dark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.05)', borderRadius: 8, padding: 3, gap: 2 },
    tab:       { flex: 1, padding: '8px 0', borderRadius: 6, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'transparent', color: sub, fontFamily: "'Sora', sans-serif", transition: 'all 0.15s' },
    tabActive: { background: card, color: dark ? '#e2e8f0' : '#0f172a', boxShadow: dark ? '0 1px 4px rgba(0,0,0,0.3)' : '0 1px 4px rgba(0,0,0,0.08)', border: `1px solid ${border}` },
    field:     { marginBottom: 18 },
    label:     { display: 'block', fontSize: 12, fontWeight: 600, color: dark ? '#94a3b8' : '#64748b', marginBottom: 7, textTransform: 'uppercase', letterSpacing: 0.5 },
    input:     { width: '100%', padding: '11px 14px', background: dark ? '#0b1120' : '#f8fafc', border: `1px solid ${border}`, borderRadius: 9, fontSize: 14, color: dark ? '#e2e8f0' : '#0f172a', boxSizing: 'border-box', fontFamily: "'Sora', sans-serif" },
