...
export default function Security() {
  ...
  function Control({ title, description }) {
    return (
      <div className="control-card" style={s.control}>
    ...
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <Section title="Incident Response" id="incidents" accent={ACCENT_COLORS.incidents}>
          ...
          <ul style={s.ul}>
            <li style={s.li}>Affected MFI Clients will be notified within <strong style={{ color: dark ? '#e2e8f0' : '#0f172a' }}>24 hours</strong> of confirmed incident detection via the email address on their account</li>
            <li style={s.li}>A detailed incident report (nature of breach, data affected, remediation steps) will be provided within <strong style={{ color: dark ? '#e2e8f0' : '#0f172a' }}>72 hours</strong></li>
...
function makeStyles(dark) {
  ...
  return {
    ...
    hero:         { position: 'relative', overflow: 'hidden', background: bg, padding: '5rem 2rem 4rem', borderBottom: `1px solid ${dark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.06)'}` },
    ...
    h1:           { fontSize: 36, fontWeight: 800, color: text, margin: '14px 0 18px', letterSpacing: -0.8 },
    ...
    badge:        { background: card, border: '1px solid', borderRadius: 10, padding: '14px 22px', textAlign: 'center', minWidth: 110, boxShadow: dark ? 'none' : '0 2px 8px rgba(0,0,0,0.06)' },
    badgeTitle:   { fontSize: 15, fontWeight: 800, marginBottom: 4, fontFamily: "'DM Mono', monospace" },
    badgeSub:     { fontSize: 11, color: '#475569' },
    section:      { marginBottom: 52, paddingBottom: 52, borderBottom: `1px solid ${dark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'}` },
    h2:           { fontSize: 20, fontWeight: 700, color: text, margin: 0 },
    grid:         { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 },
    control:      { background: card, border: `1px solid ${border}`, borderRadius: 12, padding: '1.25rem 1.5rem', transition: 'border-color 0.15s', boxShadow: dark ? 'none' : '0 2px 8px rgba(0,0,0,0.06)' },
    controlTitle: { fontSize: 13, fontWeight: 700, color: dark ? '#e2e8f0' : '#0f172a', marginBottom: 8 },
    ...
    contact:      { position: 'relative', overflow: 'hidden', background: card, border: `1px solid ${border}`, borderRadius: 16, padding: '2.5rem', textAlign: 'center', marginTop: 16, boxShadow: dark ? 'none' : '0 2px 12px rgba(0,0,0,0.06)' },
    ...
    contactTitle: { fontSize: 22, fontWeight: 800, color: text, marginBottom: 10 },
